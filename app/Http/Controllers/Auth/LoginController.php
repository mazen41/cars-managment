    ...
    */
    ...
    public function handleAppleCallback(Request $request)
    {
        ...
        if ($existingUserByProviderId) {
            ...
            //proceed to login
            auth('web')->login($existingUserByProviderId, true);
        } else {
            ...
            auth('web')->login($existing_or_new_user, true);
        }

        if (session('temp_user_id') != null) {
            Cart::where('user_id', auth('web')->user()->id)->delete(); // If previous data is available for this user, delete first
    ...
    public function handleProviderCallback(Request $request, $provider)
    {
        ...
        if ($existingUserByProviderId) {
            ...
            if ($existingUser) {
                ...
                //proceed to login
                auth('web')->login($existing_User, true);
            } else {
                //create a new user
                $newUser = new User;
        ...
        if (session('temp_user_id') != null) {
            ...
            Cart::where('user_id', auth('web')->user()->id)->delete();
    ...
    {
        if (session('temp_user_id') != null) {
            if(auth('web')->user()->user_type == 'customer'){
                Cart::where('temp_user_id', session('temp_user_id'))
                ->update(
                    [
                        'user_id' => auth('web')->user()->id,
        ...
        if (auth('web')->user()->user_type == 'admin' || auth('web')->user()->user_type == 'staff') {

            return redirect()->route('admin.dashboard');
        }
        elseif (auth('web')->user()->user_type == 'seller') {
    ...
    {
        if (auth('web')->user() != null && (auth('web')->user()->user_type == 'admin' || auth('web')->user()->user_type == 'staff')) {
    ...
    public function account_deletion(Request $request)
    {
        ...
        // if (auth('web')->user()->provider) {
        //     $social_revoke =  new SocialRevoke;
        //     $revoke_output = $social_revoke->apply(auth('web')->user()->provider);
        ...
        User::destroy(auth('web')->user()->id);

        auth('web')->guard()->logout();
        $request->session()->invalidate();
    ...
    {
        $this->middleware('guest')->except(['logout', 'leave','account_deletion']);
    }
