...
class EarningReportController extends Controller
{
    ...
    public function index()
    {
        ...
        // Payouts data
        $seller_payments = Payout::groupBy('time')->where('payment_method','!=','Seller paid to admin')
                        ->select(DB::raw('SUM(amount) as total'), DB::raw('DATE_FORMAT(created_at, "%M") AS time'))
                        ->whereYear('created_at', Carbon::now()->year)
                        ->orderBy(DB::raw('MONTH(created_at)'), 'asc')
                        ->get();
        $total_seller_payment_amount = Payout::where('payment_method','!=','Seller paid to admin')->sum('amount');
    ...
    public function net_sales(Request $request)
    {
        ...
        $admin_ids = \App\Models\User::where('user_type', 'admin')->orWhere('user_type', 'staff')->pluck('id');
        $product_sale_query = $product_sale_query->whereIn('seller_id', $admin_ids);
    ...
    public function payouts(Request $request)
    {
        $intervalType = $request->interval_type;
        // Seller payout
        $seller_payout = Payout::where('payment_method','!=','Seller paid to admin');
        if ($intervalType == 'DAY') {
    ...
    function payout_analytic(Request $request)
    {
        $intervalType = $request->interval_type;
        // Seller payments
        $seller_payment_query = Payout::groupBy('time')->where('payment_method','!=','Seller paid to admin')->whereYear('created_at', Carbon::now()->year);
        if($intervalType == 'MONTH'){
