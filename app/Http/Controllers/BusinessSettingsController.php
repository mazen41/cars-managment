...
use App\Utility\PaymentUtility\FloosakUtility;
use Carbon\Carbon;
...
class BusinessSettingsController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware(['permission:payment_methods_configurations'])->only(['payment_method', 'change_jaib_password', 'get_new_floosak_key','check_floosak_key','verify_floosak_key']);
        ...
        $this->middleware(['permission:manage_auction_settings'])->only('auction_settings');
    }
    ...
    public function payment_method(Request $request)
    {
        ...
        $payment_methods = PaymentMethod::whereNull('addon_identifier')->whereIn('name', $payment_methods_names )->get();
    ...
    public function change_jaib_password(Request $request){

       try{
        $jaibUtility = new JaibUtility;
       $response = $jaibUtility->change_password($request->current_pass, $request->new_pass);

        if($response->error){
            flash($response->error->message)->error();
        } else if($response->result->status){
            flash(translate('Password changed successfully'))->success();
            $this->overWriteEnvFile('JAIB_PASS', $request->new_pass);
        }
       }catch(\Exception $e){
    ...
    public function get_new_floosak_key(){
        $apiKey = ApiKeys::where('service', 'floosak')->first();

        if(!$apiKey || $apiKey->expiry_date->isPast()){
            try{
                $floosak = new FloosakProvider();
                $request = $floosak->generateKeyRequest();
            }catch(\Exception $e){
                return response()->json(['success'=> false , 'message'=> $e->getMessage()]);
            }
        } else {
            return response()->json(['success' => false, 'message' =>translate('key is still valid')]);
        }
        return response()->json(['success' => true, 'request_id' => $request['request_id']]);
    }

    public function verify_floosak_key(Request $request){
        ...
        try{
            $floosak = new FloosakProvider;
            $response =  $floosak->verifyGeneratedKey($request_id, $otp);
            $this->overWriteEnvFile('FLOOSAK_API_KEY', $response['key']);
            ...
            $wallets = $response->account->wallets;

       foreach($wallets as $wallet){
                FloosakWallet::updateOrCreate(['wallet_id'=> $wallet->id],[
                    'currency_id' => $wallet->currency->id,
                    'currency_name'=> $wallet->currency->name->en,
                    'currency_symbol'=> $wallet->currency->symbol->en
                ]);

       }
       return response()->json(['success'=> true , 'message'=> 'Key fetched successfully']);
        }catch(\Exception $e){
            return response()->json(['success'=> false , 'message'=> $e->getMessage()]);
        }
    }
    public function check_floosak_key(){
        $apiKey = ApiKeys::where('service', 'floosak')->first();
        if(!$apiKey){
            return response()->json(['result'=> false, 'message'=> translate('No key found')]);
        } else if($apiKey->expiry_date->isPast()){
            return response()->json(['result'=> false, 'message'=> translate('Key has expired at').' '.$apiKey->expiry_date->format('d/m/Y')]);
        } else{
            return response()->json(['result'=> true, 'message'=> translate('Key expires at').' '.$apiKey->expiry_date->format('d/m/Y')]);
        }
    ...
    {
        return view('backend.setup_configurations.wallet_settings');
    }

    public function auction_settings(){
        return view('backend.auctions.settings');
    }
