...
class CouponController extends Controller
{
    ...
    public function couponList(Request $request)
    {
        ...
        if($user){
            $user_coupons = Coupon::where('start_date', '<=', strtotime(date('d-m-Y')))
                ->where('end_date', '>=', strtotime(date('d-m-Y')))
                ->whereHas('users', function($query) use ($user) {
                    $query->where('users.id', $user->id);
                });
                $general_coupons = Coupon::where('start_date', '<=', strtotime(date('d-m-Y')))
                    ->where('end_date', '>=', strtotime(date('d-m-Y')))
                    ->whereDoesntHave('users');
                $coupons = $this->mergeAndPaginateCoupons($user_coupons, $general_coupons, 10);
        } else {
                $coupons = Coupon::where('start_date', '<=', strtotime(date('d-m-Y')))
                    ->where('end_date', '>=', strtotime(date('d-m-Y')))
                    ->whereDoesntHave('users')->paginate(10);
        }
        return new CouponCollection($coupons);
    }

    private function mergeAndPaginateCoupons($user_coupons_query, $general_coupons_query, $per_page = 10)
    {
        $user_coupons = $user_coupons_query->get();
        $general_coupons = $general_coupons_query->get();

        $merged = $user_coupons->concat($general_coupons)
            ->sortByDesc('created_at')
            ->values();

        $total = $merged->count();
        $page = request()->get('page', 1);
        $items = $merged->forPage($page, $per_page);
