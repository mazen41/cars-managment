...
class CheckoutController
{
    public function apply_coupon_code(Request $request)
    {
        ...
        //check if coupon is restricted for specific users
        if ($coupon->users()->exists()) {
            if (!$coupon->users->contains($user_id)) {
                return response()->json([
                    'result' => false,
                    'message' => translate('You are not allowed to use this coupon!')
                ]);
            }
        }

        $coupon_owner = $coupon->user;
        if ($coupon_owner == null) {
            return response()->json([
                'result' => false,
                'message' => translate('This coupon is not applicable to your cart products!')
            ]);
        } else {
            if ($coupon_owner->user_type == 'admin' || $coupon_owner->user_type == 'staff') {
                $owners = \App\Models\User::where('user_type', 'admin')->orWhere('user_type', 'staff')->pluck('id')->toArray();
            } else {
                $owners = [$coupon_owner->id];
            }
        }
        $cart_items = Cart::where('user_id', $user_id)->whereIn('owner_id', $owners)->active()->get();
        ...
        if ($coupon_discount > 0) {
            ...
            $cart_query->whereIn('owner_id', $owners)->active()->update([
