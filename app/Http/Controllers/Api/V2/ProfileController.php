...
class ProfileController extends Controller
{
    ...
    public function update(Request $request)
    {
        try{
            ...
            return response()->json([
                'result' => true,
                'should_verify_phone'   => ($user->phone != null && $user->phone_verified_at == null),
    ...
    public function updateImage(Request $request)
    {
        ...
        try {
            ...
            if ($user->avatar_original != null) {
                $uploadService->deleteImage($user->avatar_original);
            }
            $image = $request->image;

            $upload = $uploadService->uploadImage($image, 'profile');

            $user->avatar_original = $upload;
            $user->save();

            return response()->json([
                'result' => true,
                'message' => translate("Image updated"),
                'path' => uploaded_asset($upload)
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'result' => false,
