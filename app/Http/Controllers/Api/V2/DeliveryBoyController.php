...
class DeliveryBoyController extends Controller
{
    ...
    public function dashboard_summary($id)
    {
        ...
        $orderQuery = function ($status = null, $cancelRequest = null) use ($id) {
            return Order::where('assign_delivery_boy', $id)
                ->when($status, function ($q) use ($status) {
                    if (is_array($status)) {
                        if (isset($status['not_in'])) {
                            $q->whereNotIn('delivery_status', $status['not_in']);
                        } else {
                            $q->whereIn('delivery_status', $status);
                        }
                    } else {
                        $q->where('delivery_status', $status);
                    }
                })
                ->when(!is_null($cancelRequest), function ($q) use ($cancelRequest) {
                    $q->where('cancel_request', $cancelRequest);
                });
        };


        return response()->json([
            'completed_delivery' => $orderQuery('delivered')->count(),

            'pending_delivery' => $orderQuery(['not_in' => ['delivered', 'cancelled']], '0')->count(),
            'cancelled' => $orderQuery('cancelled')->count(),
            'on_the_way' => $orderQuery('on_the_way', '0')->count(),
            'picked' => $orderQuery('picked_up', '0')->count(),
            'assigned' => $orderQuery(['pending', 'confirmed'], '0')->count(),
    ...
    public function assigned_delivery($id)
    {
        ...
        return new DeliveryBoyPurchaseHistoryMiniCollection($order_query->latest()->paginate(10));
    }
     ...
     *
     * @param int $id
     * @return DeliveryBoyPurchaseHistoryMiniCollection
     */
    ...
    public function completed_delivery($id)
{
    ...
    if (request()->has('date_range') && request()->date_range != null) {
        ...
        $order_query->whereBetween('delivery_history_date', [$min_date, $max_date]);
    }
    ...
    public function cancelled_delivery($id)
{
    ...
    public function change_delivery_status(Request $request)
    {
        ...
        $order->save();
        $order->deliveryHistory()->save($delivery_history);
        ...
        if (get_setting('google_firebase') == 1 && $order->user->device_token != null) {
            ...
            $notification['text'] = translate("Delivery status changed for order") . ": " . $order->code . " " . translate('to') . " " . translate($status);

            $notification['type'] = "order";
            $notification['id'] = $order->id;
            $notification['user_id'] = $order->user->id;

            NotificationUtility::sendFirebaseNotification((object)$notification);
        }
        return response()->json([
            'result' => true,
