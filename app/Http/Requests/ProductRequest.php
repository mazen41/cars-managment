...
class ProductRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $rules['specifications']  = 'array|max:30';
        $rules['specifications.*.key']  = 'required_with:specifications.*.value|string|max:255';
        $rules['specifications.*.value']  = 'required_with:specifications.*.key|string|max:255';
    ...
    public function prepareForValidation()
    {
        ...
        $this->merge([
            'category_ids' => $this->category_id ? [$this->category_id] : [],
        ]);
        if($this->subcategory_id != null){
            $this->merge([
                'category_ids' => [$this->subcategory_id, $this->category_id]
            ]);
        }
    }
}
