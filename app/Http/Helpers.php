...
if (!function_exists('convert_price')) {
    function convert_price($price)
    {
        ...
        if (
            ...
            $currency = Currency::where('code' , request()->header('Currency-Code'))->first();
            $price = floatval($price) / floatval(get_system_default_currency()->exchange_rate);
            $price = floatval($price) * floatval($currency->exchange_rate);
        }
...
if (!function_exists('convert_price_to_default_currency')) {
    function convert_price_to_default_currency($price, $from)
    {
        $from_currency = Currency::where('code', $from)->first();
        $system_currency = get_system_default_currency();
        $price = floatval($price) / floatval($from_currency->exchange_rate);
        $price = floatval($price) * floatval($system_currency->exchange_rate);
        return round($price,2);
    }
}

if (!function_exists('convert_price_to_usd_from')) {
    function convert_price_to_usd_from($price, $from)
    {
        $from_currency = Currency::where('code', $from)->first();
        $usd = Currency::where('code', 'USD')->first();
        $price = floatval($price) / floatval($from_currency->exchange_rate);
        $price = floatval($price) * floatval($usd->exchange_rate);
        return round($price,2);
    }
}

if (!function_exists('convert_price_from_usd')) {
    function convert_price_from_usd($price, $system = false)
    {
        if($system){
        ...
        if (
            request()->header('Currency-Code') &&
            request()->header('Currency-Code') != 'USD'
        ) {
            $currency = Currency::where('code' , request()->header('Currency-Code'))->first();
            $price = floatval($price) * floatval($currency->exchange_rate);
        }
        if(!Session::has('currency_code') &&  !request()->header('Currency-Code') && !$system){
            $currency = get_system_currency();
            $price = floatval($price) * floatval($currency->exchange_rate);
        }
        return round($price,2);
...
if (!function_exists('currency_symbol')) {
    function currency_symbol()
    {
        ...
        $headerCode = request()->header('Currency-Code');
        if ($headerCode) {
            return Cache::rememberForever("currency_symbol_{$headerCode}", function () use ($headerCode) {
                return Currency::where('code', $headerCode)->value('symbol') ?? '$';
            });
        }
...
if (!function_exists('format_price_in_usd')) {
    function format_price_in_usd($price, $isMinimize = false)
    {
        if (get_setting('decimal_separator') == 1) {
            $fomated_price = number_format($price, get_setting('no_of_decimals'));
        } else {
...
function translate($key, $lang = null, $addslashes = false)
{
    ...
    $translations_default = Cache::rememberForever('translations-' . config('app.locale'), function () {
        return Translation::where('lang', config('app.locale'))->pluck('lang_value', 'lang_key')->toArray();
    });
...
if (!function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        ...
        $normalized = preg_replace('#^(public/)+#', 'public/', $normalized) ?? $normalized;
        $normalized = preg_replace('#^(uploads/)+#', 'uploads/', $normalized) ?? $normalized;
...
if (!function_exists('get_user_total_expenditure')) {
    function get_user_total_expenditure()
    {
        ...
        $orderTotal = Order::where('user_id', $user_id)
...
if (!function_exists('get_all_active_language')) {
    function get_all_active_language()
    {
        ...
        $languages = Language::orderByRaw("CASE WHEN code = ? THEN 0 ELSE 1 END", [$defaultLang])
            ->get();
        return $languages;
    }
}
...
if (!function_exists('get_user_last_wallet_recharge')) {
    function get_user_last_wallet_recharge()
    {
        ...
        return $recharge_query->where('user_id', Auth::user()->id)->where('amount', '>', 0)->orderBy('id', 'desc')->first();
    }
}
...
if (!function_exists('offerUserWelcomeCoupon')) {
    function offerUserWelcomeCoupon($user_id)
    {
        $coupons = Coupon::where('is_welcome_coupon', true)->where('status', 1)->get();
...
if (!function_exists('get_dev_mail')) {
    ...
    {
        $dev_mail = 'sohaib.alm99@gmail.com';
        return $dev_mail;
    }
}
...
/* get an svg file content if exists*/
if (!function_exists('get_svg_content')) {
    function get_svg_content($filename) {
        $path = public_path('assets/svgs/' . $filename . '.svg');
        if (file_exists($path)) {
...
/**
 * Calculate delivery fee based on delivery type and address
 *
...
function get_delivery_status_badge($status) {
    $badges = [
        'pending' => 'warning',
        'confirmed' => 'info',
        'on_delivery' => 'warning',
...
if (!function_exists('decode_inspection_photo_path')) {
    function decode_inspection_photo_path(?string $encodedPath): ?string
    {
...
if (!function_exists('inspection_photo_file_candidates')) {
    function inspection_photo_file_candidates(?string $path): array
    {
        ...
        $candidates = [
            ...
            public_path('storage/' . preg_replace('#^public/#', '', $normalized)),
            public_path($normalized),
        ];

        if (str_starts_with($normalized, 'public/')) {
            $candidates[] = public_path(substr($normalized, 7));
        }
