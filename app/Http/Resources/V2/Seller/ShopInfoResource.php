...
class ShopInfoResource extends JsonResource
{
    public function toArray($request)
    {
        ...
        [
            ...
            'orders'=> $this->user->seller_orders()->where('payment_status', "paid")->count(),
            'cars' => $this->user->cars()->count(),
            'sales'=>format_price( $this->user->seller_sales()->where("payment_status","paid")->sum('price'),true),
            "car_reservations_count" => $this->car_reservations_count ?? 0,
            "car_inspections_count"  => $this->car_inspections_count ?? 0,
        ];
    }
