        ...
        return [
            'data' => $this->collection->map(function ($data) {
                ...
                if (addon_is_activated('refund_request')) {
                    ...
                    $last_refund_date = $data->created_at->addDays((int)$no_of_max_day);
                    ...
                    if (
                        ...
                        $refund_label = translate("Pending");
                        $refund_request_status = intval($data->refund_request->refund_status);
                    } else if ($data->refund_request != null && $data->refund_request->refund_status == 2) {
                        $refund_label = translate("Rejected");
                        $refund_request_status = intval($data->refund_request->refund_status);
                    } else if ($data->refund_request != null && $data->refund_request->refund_status == 1) {
                        $refund_label = translate("Approved");
                        $refund_request_status = intval($data->refund_request->refund_status);
                    } else if ($data->product->refundable != 0) {
                        $refund_label = translate("N/A");
                    } else {
                        $refund_label = translate("Non-refundable");
                    }
                }
                return [
                    'id' => $data->id,
