        ...
        <div class="card">
            <div class="card-body">
                <div class="card-title text-primary mb-3">
                    <h6 class="mb-0">{{ translate('Profile Completion Status') }}</h6>
                </div>

                @php
                    $shop = Auth::user()->shop;
                    $profile_complete = $shop->hasCompletedProfileInfo();
                    $payment_complete = $shop->hasCompletedPaymentInfo();
                    $delivery_complete = $shop->hasCompletedDeliveryInfo();
                    $all_complete = $profile_complete && $payment_complete && $delivery_complete;
                @endphp

                @if(!$all_complete)
                    <div class="alert alert-warning mb-4" role="alert">
                        <i class="las la-exclamation-triangle"></i>
                        {{ translate('Important: You cannot post products or request withdrawals until you complete your shop information.') }}
                        <a href="{{ route('seller.shop.index') }}" class="alert-link">{{ translate('Complete Setup') }}</a>
                    </div>
                @endif

                <div class="row gutters-10">
                    <div class="col-md-4">
                        <div class="border border-{{ $profile_complete ? 'success' : 'danger' }} rounded p-3 text-center mb-3">
                            <i class="las la-user-circle fs-24 {{ $profile_complete ? 'text-success' : 'text-danger' }} mb-2"></i>
                            <div class="fs-14 fw-600">{{ translate('Profile Information') }}</div>
                            <div class="fs-12">
                                @if($profile_complete)
                                    <span class="badge badge-inline  badge-success">{{ translate('Complete') }}</span>
                                @else
                                    <span class="badge badge-inline  badge-danger">{{ translate('Incomplete') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="border border-{{ $payment_complete ? 'success' : 'danger' }} rounded p-3 text-center mb-3">
                            <i class="las la-credit-card fs-24 {{ $payment_complete ? 'text-success' : 'text-danger' }} mb-2"></i>
                            <div class="fs-14 fw-600">{{ translate('Payment Information') }}</div>
                            <div class="fs-12">
                                @if($payment_complete)
                                    <span class="badge badge-inline  badge-success">{{ translate('Complete') }}</span>
                                @else
                    ...
                    <div class="col-md-4">
                        <div class="border border-{{ $delivery_complete ? 'success' : 'danger' }} rounded p-3 text-center mb-3">
                            <i class="las la-truck fs-24 {{ $delivery_complete ? 'text-success' : 'text-danger' }} mb-2"></i>
                            <div class="fs-14 fw-600">{{ translate('Delivery Information') }}</div>
                            <div class="fs-12">
            ...
            <div
                ...
                @if (Auth::user()->shop->verification_status == 0)
                    <div class="my-n4 py-1 text-center">
                        <img src="{{ static_asset('assets/img/not_verified.png') }}" alt=""
                ...
                @else
                    <div class="my-2 py-1">
                        <img src="{{ static_asset('assets/img/verified_.png') }}" alt="" width="">
                    </div>
                @endif
            </div>
        </div>
    ...
    <div class="row">
        ...
        <div class="col-sm-6 col-md-6 col-lg-3">
            <div class="card mb-4 p-4 text-center bg-soft-primary">
                ...
                <a href="{{ route('seller.shop.index') }}#payment-settings" class="btn btn-primary">
            ...
            <div class="aiz-carousel gutters-10 half-outside-arrow" data-items="6" data-xl-items="5" data-lg-items="4"
                ...
                @foreach ($products as $key => $product)
                    <div class="carousel-box">
                        <div
                            class="aiz-card-box border border-light rounded shadow-sm hov-shadow-md mb-2 has-transition bg-surface">
...
@section('script')
    <script type="text/javascript">
        if ({{!auth()->user()->shop->cash_on_delivery_status}} && {{!auth()->user()->shop->bank_payment_status}} ){
            AIZ.plugins.notify('danger', "{{translate('You need to set your payemnt details')}}");
        }
