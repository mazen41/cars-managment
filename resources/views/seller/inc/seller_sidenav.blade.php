    ...
    <div class="aiz-sidebar left c-scrollbar">
        ...
        <div class="aiz-side-nav-wrap">
            ...
            <ul class="aiz-side-nav-list" id="main-menu" data-toggle="aiz-side-menu">
                ...
                @endif
                    @php
                        $seller_orders = \App\Models\Order::where('viewed_by_seller', 0)->where('seller_id', \Auth::user()->id)->count();
                    @endphp
                <li class="aiz-side-nav-item">
                    <a href="{{ route('seller.orders.index') }}"
                        ...
                        @if (isset($seller_orders) && $seller_orders > 0)
                                <span class="badge badge-success">{{ $seller_orders }}</span>
                            @endif
                    </a>
                </li>
                ...
                <li class="aiz-side-nav-item">
                 ...
                 @if (get_setting('conversation_system') == 1)
                    @php
                       $conversation = \App\Models\Conversation::where('receiver_id', Auth::user()->id)->where('receiver_viewed', '0')->count();
                       $queries = \App\Models\ProductQuery::where('seller_id', Auth::user()->id)->where('reply', null)->count();
                    @endphp
                    <li class="aiz-side-nav-item">
               ...
               {{-- @if (get_setting('product_query_activation') == 1)
                    <li class="aiz-side-nav-item">
                        <a href="{{ route('seller.product_query.index') }}"
                            ...
                            @if ($queries > 0)
                            <span class="badge badge-success">{{ $queries }}</span>
                        @endif
                        </a>
                    </li>
                @endif --}}
