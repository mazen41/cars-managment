...
<div class="card">
    ...
    <div class="card-body">
        <div class="row gutters-5">
            ...
            <div class="col-md-3 ml-auto">
                ...
                {{-- <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity"
                    ...
                    <select class="form-control aiz-selectpicker" data-minimum-results-for-search="Infinity"
                        id="update_payment_status" onchange="confirm_payment_status(this.value)">
        ...
        <div class="row gutters-5">
            <div class="col text-md-left text-center">
                ...
                <address>
                    ...
                    json_decode($order->shipping_address)->state }} - @endif <br>
                ...
                {{ translate('Name') }}: {{ $order->manual_payment_data->name }}<br>
                {{ translate('Method name') }}: {{ $order->manual_payment_data->method_name }}<br>
                {{ translate('Amount') }}:
                {{ single_price($order->manual_payment_data->amount) }}<br>
                {{ translate('TRX ID') }}: {{ $order->manual_payment_data->trx_id }}
                <br>
                @if ($order->manual_payment_data->photo)
                <a href="{{ uploaded_asset($order->manual_payment_data->photo) }}" target="_blank">
                    <img src="{{ uploaded_asset($order->manual_payment_data->photo) }}" alt="" height="100">
                </a>
                @endif
                @endif
            </div>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="{{ route('products.admin.edit', $orderDetail->product->slug) }}" target="_blank">
                            ...
                            <td>
                                ...
                                <small>
                                    @php
                                    $product_stock = $orderDetail->product->stocks->where('variant',
                                    $orderDetail->variation)->first();
                                    @endphp
        ...
        @if ($order->sibling_orders->count() > 0)
        <strong class="text-main">{{translate('Payment shared orders')}}</strong>
        <div class="">
            @foreach ($order->sibling_orders as $sibling)
            <a href="{{route('orders.show', encrypt($sibling->id))}}" target="_blank">{{$sibling->code}}</a><br>
            @endforeach
        </div>
        @endif
...
<script type="text/javascript">
    $('#assign_deliver_boy').on('change', function() {
        ...
        var selected_payment_status = null;

        function confirm_payment_status(value) {
            selected_payment_status = value;
        ...
        function update_payment_status(){
            ...
            $.post('{{ route('orders.update_payment_status') }}', {
                _token: '{{ csrf_token() }}',
                order_id: order_id,
                status: selected_payment_status
            }, function(data) {
