    ...
    <div class="aiz-titlebar text-left mt-2 pb-2 px-3 px-md-2rem border-bottom border-gray">
        <div class="row align-items-center">
            ...
            <div class="col text-right">
              <a href="#" onclick="history.back()" class="btn btn-light btn-sm mr-3">
            {{ translate('Back to list') }} <i class="las la-arrow-left"></i>
        </a>
        </div>
        </a>
            {{-- <div class="col text-right">
                            ...
                            <div class="w-100">
                                <div class="row">
                                    ...
                                    <div class="col-xxl-5 col-xl-6">
                                        <div class="card p-4 @if($errors->has('category_ids') || $errors->has('category_id')) border border-danger @endif">
                                            <div class="card-header">
                                                <h5 class="mb-0 h6">{{ translate('Product Category') }}</h5>
                                            </div>
                                            <div class="form-group">
                                                  <label class="fs-13">{{ translate('Select Main') }} <span class="text-danger">*</span></label>

                                            <select class="select2 form-control aiz-selectpicker" id="category_id" name="category_id" onchange="getSubcategories()" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                                            ...
                                            {{-- SubCategoeis --}}
                                             <div class="form-group">
                                                  <label class="fs-13">{{translate('Subcategory')}}</label>
                                                <select class="select2 form-control aiz-selectpicker" onchange="getBrands()" id="subcategory_id" name="subcategory_id" data-toggle="select2" data-placeholder="Choose ..." data-live-search="true">
                                                    <option value="0">{{ translate('Select main category first') }}</option>
                                                </select>
                                            </div>
                                        </div>
                                        <!-- Brand -->
                                        <div class="form-group row" id="brand">
                                            <label class="col-xxl-2 col-from-label fs-13">{{translate('Brand')}}</label>
                            ...
                            <div class="w-100">
                               ...
                               <div class="card shadow-sm">
                                    <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center">
                                        <h5 class="mb-0">{{ translate('Product Specifications') }}</h5>
                                        <button type="button" class="btn btn-light btn-sm" id="add-spec">
                                            ...
                                            @if ($product->specifications != null)
                                                @foreach ($product->specifications as $key => $value)
                                                    <div class="row g-2 mb-2 spec-row">
                                                        <div class="col-md-5">
                                                            <input type="text" name="specifications[{{ $key }}][key]" class="form-control" placeholder="e.g. Battery" value="{{ $value['key'] }}" required>
                                                        </div>
                                                        <div class="col-md-6">
                                                            <input type="text" name="specifications[{{ $key }}][value]" class="form-control" placeholder="e.g. 5000 mAh" value="{{ $value['value'] }}" required>
                                                        </div>
                                                        <div class="col-md-1">
                                                        ...
                                                        <table class="table mb-0">
                                                            ...
                                                            <tbody>
                                                                @foreach($fq_bought_products as $fQBproduct)
                                                                    @if (!isset($fQBproduct->frequently_bought_product))
                                                                        @continue
                                                                    @endif
                                                                    <tr class="remove-parent">
                                                                        ...
                                                                        <td style="vertical-align: middle;">
                                                                            <p class="d-block fs-13 fw-700 hov-text-primary mb-1 " title="{{ translate('Product Name') }}">
...
<script type="text/javascript">
    ...
    function getSubcategories(callback) {
        ...
        if(category_id) {
            ...
            $.ajax({
                url: "{{ route('categories.get-sub-categories') }}",
                type: 'GET',
                data: { category_id: category_id },
                success: function(data) {
                    subCategorySelect.removeAttr('disabled');
                    subCategorySelect.append('<option value="">{{ translate('Select') }}</option>');
            ...
            $('#subcategory_id').empty().append('<option value="">{{ translate('Select Subcategory') }}</option>').selectpicker('refresh');
        }
    }
    ...
    function getBrands() {
        ...
        if(subcategory_id != '' && subcategory_id != 0) {
            ...
            $.ajax({
                url: "{{ route('categories.get-category-brands') }}",
                type: 'GET',
                data: { category_id: subcategory_id },
                success: function(data) {

                    brandSelect.append('<option value="">{{ translate('Select') }}</option>');
            ...
            $('#brand_id').empty().append('<option value="">{{ translate('Select Brand') }}</option>').selectpicker('refresh');
        }
    }
    ...
    $('#add-spec').on('click', function() {
        const rowCount = $('.spec-row').length;

        // Safety check for the limit we discussed (e.g., 30)
        if (rowCount >= 30) {
            alert('Maximum 30 specifications allowed.');
            return;
        }

        const newRow = `
            <div class="row g-2 mb-2 spec-row" style="display: none;">
                <div class="col-md-5">
                    <input type="text" name="specifications[${specIndex}][key]" class="form-control" placeholder="e.g. Battery" required>
                </div>
                <div class="col-md-6">
                    <input type="text" name="specifications[${specIndex}][value]" class="form-control" placeholder="e.g. 5000 mAh" required>
                </div>
                <div class="col-md-1">
                    <button type="button" class="btn btn-outline-danger w-100 remove-spec">
                        <i class="las la-trash"></i>
        ...
        $(newRow).appendTo('#spec-container').fadeIn(200);
        specIndex++;
    });

    // Remove row (using delegation for dynamically added elements)
    $(document).on('click', '.remove-spec', function() {

        $(this).closest('.spec-row').fadeOut(200, function() {
            $(this).remove();
        });
    });
