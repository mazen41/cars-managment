        ...
        <div class="card">
            <div class="card-body p-0">
                ...
                <form class="p-4" action="{{ route('categories.update', $category->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    {{-- Brands for subcategories --}}
                    <div class="form-group row @if($category->parent_id == null) d-none @endif">
                        <label class="col-md-3 col-form-label">{{translate('Brands')}}</label>
                        <div class="col-md-9">
                            <select class="select2 form-control aiz-selectpicker" name="brands[]" data-toggle="select2" data-placeholder="Choose ..."data-live-search="true" data-selected="{{ $category->brands->pluck('id') }}" multiple>
...
<script type="text/javascript">
    ...
    // hide brands when a parent category is not selected
    $(document).ready(function() {
        $('select[name="parent_id"]').on('change', function() {
            if($(this).val() == '' || $(this).val() == 0){
                $('select[name="brands[]"]').closest('.form-group').addClass('d-none');
            } else {
                $('select[name="brands[]"]').closest('.form-group').removeClass('d-none');
            }
        });
    });
</script>
