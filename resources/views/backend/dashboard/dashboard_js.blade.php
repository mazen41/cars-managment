<script>
    ...
    $('#customers_data_tab').on('shown.bs.tab', function (e) {
        if (!customersDataLoaded) {
            loadCustomersStats();
        }
    });
    ...
    function loadCustomersStats() {
        $.ajax({
            url: '{{ route("admin.dashboard.customers_data") }}',
            type: 'GET',
            success: function(response) {
                $('#customers_data').html(response);
                customersDataLoaded = true;
                initializeCustomerGrowthChart();
                initializeTopCitiesCharts();
                initializeCustomerRetentionChart();
            },
            error: function() {
                $('#customers_data').html('<div class="alert alert-danger">{{ translate("Error loading customers data") }}</div>');
            }
        });
    }
});
