...
<form action="{{ route('admin.car-inspections.update', $carInspection->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="row">
        <div class="col-lg-8">
            <div class="card">
                ...
                <div class="card-body">
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">{{ translate('Car') }}</label>
                        <div class="col-md-9">
                                <div class='d-flex align-items-center'>
                                    @if($selectedCar->main_photo)
                                        <img src='{{ uploaded_asset($selectedCar->main_photo) }}' class='size-30px img-fit rounded mr-2'>
                                    @else
                    ...
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label">{{ translate('Inspection Type') }}</label>
                        <div class="col-md-9">
                            <div class='d-flex align-items-center'>
                                <div class='size-30px bg-soft-secondary rounded mr-2 d-flex align-items-center justify-content-center'>
                                    <i class='las la-clipboard-list text-secondary'></i>
                                </div>
                            <div>
                                <div>{{ $inspectionType->name }}</div>
                            </div>
                        </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <select class="form-control aiz-selectpicker" name="inspector_id" id="inspector_id" data-live-search="true" @if($selectedInspector) disabled @endif>
                                ...
                                @foreach($inspectors as $inspector)
                                    ...
                                    " @if(isset($selectedInspector)) @if($inspector->id == $selectedInspector->id) selected @endif @endif>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input type="datetime-local"
                                   ...
                                   id="scheduled_at"
                                   min="{{ date('Y-m-d\TH:i') }}"
                                   value="{{ old('scheduled_at', $carInspection->scheduled_at ? $carInspection->scheduled_at->format('Y-m-d\TH:i') : '') }}"
                                   required>
