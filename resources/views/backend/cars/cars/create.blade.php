...
<div class="page-content">
    <div class="aiz-titlebar text-left mt-2 pb-2 px-3 px-md-2rem border-bottom border-gray">
        <div class="row align-items-center">
            <div class="col">
                <h1 class="h3">{{ translate('Add New Car') }}</h1>
            </div>
        </div>
    </div>
    ...
    <div class="d-sm-flex">
        ...
        <div class="flex-grow-1 p-sm-3 p-lg-2rem mb-2rem mb-md-0">
            ...
            <form action="{{ route('admin.cars.store') }}" method="POST" enctype="multipart/form-data" id="car_form" novalidate>
                @csrf
                    ...
                    <div class="tab-pane fade show active" id="general" role="tabpanel" aria-labelledby="general-tab">
                        <div class="bg-surface p-3 p-sm-2rem">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" name="vin" value="{{ old('vin') }}" maxlength="17">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control aiz-selectpicker" id="condition" name="condition" required>
                                            ...
                                            <option value="new" {{ old('condition') == 'new' ? 'selected' : '' }}>{{ translate('New') }}</option>
                                            <option value="used" {{ old('condition') == 'used' ? 'selected' : '' }}>{{ translate('Used') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                       ...
                                       <select class="form-control aiz-selectpicker" id="color_id" name="color_id" required>
                                            ...
                                            @foreach ($colors as $color)
                                                <option value="{{ $color->id }}" {{ old('color') == $color->id ? 'selected' : '' }}>{{ $color->getTranslation('name') }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control aiz-text-editor" id="description" name="description" rows="8" placeholder="{{ translate('Enter car description') }}" required>{{ old('description') }}</textarea>
                            </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control aiz-selectpicker" id="category_id" name="category_id" data-live-search="true">
                                            ...
                                            @foreach ($categories as $category)
                                                <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>{{ $category->getTranslation('name') }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" class="form-control" id="manufacture_year" name="manufacture_year" value="{{ old('manufacture_year') }}" min="1900" max="{{ date('Y') + 1 }}" placeholder="{{ translate('Enter year') }}" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="milage">{{ translate('Mileage (km)') }} <span class="text-danger">*</span></label>
                                        <input type="number" class="form-control" id="milage" name="milage" value="{{ old('milage') }}" min="0" step="0.01" placeholder="{{ translate('Enter mileage') }}" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="price">{{ translate('Price') }}</label>
                                        <input type="number" class="form-control" id="price" name="price" value="{{ old('price') }}" min="0" step="0.01" placeholder="{{ translate('Enter price') }}">
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <label for="transmission">{{ translate('Transmission') }}</label>
                                         <select class="form-control aiz-selectpicker" id="transmission" name="transmission" required>
                                            @foreach (\App\Enums\CarTransmissionTypeEnum::values() as $transmission)
                                                <option value="{{ $transmission->getValue() }}" {{ old('transmission') == $transmission->getValue() ? 'selected' : '' }}>
                                                    {{ $transmission->label() }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control aiz-selectpicker" id="fuel_type" name="fuel_type" required>
                                            @foreach (\App\Enums\CarFuelTypeEnum::values() as $fuelType)
                                                <option value="{{ $fuelType->getValue() }}" {{ old('fuel_type') == $fuelType->getValue() ? 'selected' : '' }}>
                                                    {{ $fuelType->label() }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control aiz-selectpicker" id="moderation_status" name="moderation_status" required>
                                            <option value="pending" {{ old('moderation_status') == 'pending' ? 'selected' : '' }}>{{ translate('Pending') }}</option>
                                            <option value="published" {{ old('moderation_status') == 'published' ? 'selected' : '' }}>{{ translate('Published') }}</option>
                                            <option value="rejected" {{ old('moderation_status') == 'rejected' ? 'selected' : '' }}>{{ translate('Rejected') }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select class="form-control aiz-selectpicker" id="car_status" name="car_status" required>
                                            <option value="available" {{ old('car_status') == 'available' ? 'selected' : '' }}>{{ translate('Available') }}</option>
                                            <option value="reserved" {{ old('car_status') == 'reserved' ? 'selected' : '' }}>{{ translate('Reserved') }}</option>
                                            <option value="in_auction" {{ old('car_status') == 'in_auction' ? 'selected' : '' }}>{{ translate('In Auction') }}</option>
                                            <option value="sold" {{ old('car_status') == 'sold' ? 'selected' : '' }}>{{ translate('Sold') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="bg-surface p-3 p-sm-2rem">
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="false">
                                    ...
                                    <input type="hidden" name="main_photo" class="selected-files" value="{{ old('main_photo') }}" required>
                                </div>
                        ...
                        <div class="bg-surface p-3 p-sm-2rem">
                            ...
                            @foreach ($customFields as $field)
                                <div class="form-group">
                                    ...
                                    @if($field->type == 'text')
                                        <input type="text" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'number')
                                        <input type="number" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'email')
                                        <input type="email" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'url')
                                        <input type="url" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'textarea')
                                        <textarea class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" rows="4" {{ $field->required ? 'required' : '' }}>{{ old('custom_field_' . $field->id) }}</textarea>
                                    @elseif($field->type == 'select')
                                        <select class="form-control aiz-selectpicker" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" {{ $field->required ? 'required' : '' }}>
                                            ...
                                            @foreach($field->options as $option)
                                                <option value="{{ $option->value }}" {{ old('custom_field_' . $field->id) == $option->value ? 'selected' : '' }}>{{ $option->label }}</option>
                                            @endforeach
                                        </select>
                                            ...
                                            @foreach($field->options as $option)
                                                <div class="col-md-3">
                                                    <label class="aiz-radio">
                                                        <input type="radio" name="custom_field_{{ $field->id }}" value="{{ $option->value }}" {{ old('custom_field_' . $field->id) == $option->value ? 'checked' : '' }} {{ $field->required ? 'required' : '' }}>
                                    ...
                                    @elseif($field->type == 'checkbox')
                                        <div class="row">
                                            @foreach($field->options as $option)
                                                <div class="col-md-3">
                                                    <label class="aiz-checkbox">
                                                        <input type="checkbox" name="custom_field_{{ $field->id }}[]" value="{{ $option->value }}" {{ in_array($option->value, old('custom_field_' . $field->id, [])) ? 'checked' : '' }}>
                                                        <span class="aiz-square-check"></span>
                                    ...
                                    @elseif($field->type == 'date')
                                        <input type="date" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'time')
                                        <input type="time" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'datetime')
                                        <input type="datetime-local" class="form-control" id="custom_field_{{ $field->id }}" name="custom_field_{{ $field->id }}" value="{{ old('custom_field_' . $field->id) }}" {{ $field->required ? 'required' : '' }}>
                                    @elseif($field->type == 'file')
                                        <div class="input-group" data-toggle="aizuploader" data-type="document">
                                            ...
                                            <input type="hidden" name="custom_field_{{ $field->id }}" class="selected-files" value="{{ old('custom_field_' . $field->id) }}">
                                        </div>
                    ...
                    <div class="tab-pane fade" id="location" role="tabpanel" aria-labelledby="location-tab">
                        <div class="bg-surface p-3 p-sm-2rem">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="location" name="location" value="{{ old('location') }}" placeholder="{{ translate('Enter location') }}" required>
                            </div>
                ...
                <div class="bg-surface p-3 p-sm-2rem">
                    <div class="text-right">
                        <button type="submit" class="btn btn-primary">{{ translate('Save Car') }}</button>
                    </div>
                </div>
            </form>
        </div>
        ...
        if (brand_id) {
            $.get('{{ route('admin.cars.models-by-brand', '') }}/'+brand_id, function(data) {
                ...
                for (var i = 0; i < data.models.length; i++) {
                    $('#model_id').append('<option value="' + data.models[i].id + '">' + data.models[i].name + '</option>');
                }
