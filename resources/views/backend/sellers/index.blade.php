...
<div class="card">
    <form class="" id="sort_sellers" action="" method="GET">
        <div class="card-header row gutters-5">
            ...
            @canany(['delete_seller', 'export_seller'])
                <div class="dropdown mb-2 mb-md-0">
                    ...
                    <div class="dropdown-menu dropdown-menu-right">
                        ...
                        @endcan
                        @can('export_seller')
                        <a class="dropdown-item" href="javascript:void(0)" onclick="seller_bulk_export('PDF')">{{ translate('Export PDF') }}</a>
                        <a class="dropdown-item" href="javascript:void(0)" onclick="seller_bulk_export('XLS')">{{ translate('Export XLS') }}</a>
                        <a class="dropdown-item" href="javascript:void(0)" onclick="seller_bulk_export('CSV')">{{ translate('Export CSV') }}</a>
                        @endcan
                    </div>
        ...
        <div class="card-body">
            <table class="table aiz-table mb-0">
                ...
                <tr>
                    ...
                    <th>
                        @if(auth()->user()->can('delete_seller') || auth()->user()->can('export_seller'))
                    ...
                    <th data-breakpoints="lg">{{translate('Total Paid Orders')}}</th>
                    <th data-breakpoints="lg">{{translate('Total payment amount')}}</th>
                ...
                @foreach($shops as $key => $shop)
                    <tr>
                        ...
                        <td>{{single_price($shop->total_admin_commission)}}</td>
                        <td>{{single_price($shop->paid_orders)}}</td>
                        <td>{{single_price($shop->payments->sum('amount'))}}</td>
                         <td>
                            ({{ $shop->rating_count }})
                            ...
                            <div class="dropdown">
                                ...
                                <div class="dropdown-menu dropdown-menu-right dropdown-menu-xs">
                                    ...
                                    @can('seller_payment_history')
                                        <a href="{{route('sellers.payment_histories')}}?seller_id={{$shop->user_id}}" class="dropdown-item">
            ...
            <x-table_pagination :data="$shops" :paginate="$paginate" />
        </div>
    </form>
</div>
...
@section('script')
    <script type="text/javascript">
        ...
        function seller_bulk_export (type){
            var url = '{{route('shops-export')}}';
            $("#sort_sellers").attr("action", url);
            $('input[name="export_type"]').val(type);
            $('#sort_sellers').submit();
            $("#sort_sellers").attr("action", '');
        }
    </script>
@endsection
