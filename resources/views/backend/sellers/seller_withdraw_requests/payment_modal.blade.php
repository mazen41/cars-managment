    ...
    <div class="modal-body">
        <table class="table table-striped table-bordered">
            <tbody>
                ...
                @if($user->shop->bank_payment_status == 1 && $user->shop->wallets->count() > 0)
                    <tr>
                        <td colspan="2">
                            <strong>{{ translate('Available Wallets') }}</strong>
                        </td>
                    </tr>
                    @foreach($user->shop->wallets as $wallet)
                        <tr>
                            <td colspan="2">
        ...
        @if ($user->shop->admin_to_pay > 0)
            ...
            @if($user->shop->bank_payment_status == 1 && $user->shop->wallets->count() > 0)
                <div id="wallet_selection_section" style="display: none;">
                    <div class="form-group row">
                        <label class="col-sm-3 col-from-label" for="wallet_id">{{translate('Select Wallet')}}</label>
                        <div class="col-sm-9">
                            <select name="wallet_id" id="wallet_id" class="form-control demo-select2-placeholder">
                                @foreach($user->shop->wallets as $wallet)
            ...
            @else
                <span class="text-danger">{{translate('Shop has not set their payment details')}}</span>
            @endif
