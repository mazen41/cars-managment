                ...
                @foreach($wallets as $key => $wallet)
                    @if ($wallet->user != null)
                        <tr>
                            ...
                            <td>
                                @if ($wallet->payment_details != null && !empty($wallet->payment_details))
                                    {{translate('Name')}}: {{isset($wallet->payment_details->name)? $wallet->payment_details->name : ''}}<br>
                                    {{translate('TXN ID')}}: {{isset($wallet->payment_details->trx_id)? $wallet->payment_details->trx_id : ''}}<br>
                                    {{translate('Bank name')}}: {{isset($wallet->payment_details->method_name) ? $wallet->payment_details->method_name : ''}}<br>
                                @endif
                            </td>
                            <td>
